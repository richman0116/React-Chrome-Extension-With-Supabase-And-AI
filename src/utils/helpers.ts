...
export const resizeAndConvertImageToBuffer = async (imageUrl: string): Promise<Uint8Array> => {
  // Load the image
...
export const uploadImageToSupabase = async (imageBuffer: ArrayBuffer, bucketName: string, uploadPath: string) => {
  try {

    const { error } = await supabase.storage.from(bucketName).upload(uploadPath, imageBuffer, {
